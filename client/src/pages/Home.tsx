...
function ScoreDial({ score, qualification }: { score: number; qualification: string }) {
  ...
  return <div className="score-dial" aria-label={`Qualification score ${score} out of 100, ${fitLabel}`}>
...
function QualificationSignalMap({ signals }: { signals: QualificationSignal[] }) {
  const points = [
    [50, 9], [72, 16], [89, 37], [86, 63], [67, 84], [37, 87], [14, 67], [11, 39], [29, 17], [50, 50],
  ];
  const mapSignals = signals.slice(0, 10);
  return <div className="signal-map-wrap" role="img" aria-label="Qualification signal map showing ten factors and their assessment states">
    <svg className="qualification-map" viewBox="0 0 100 100" aria-hidden="true">
      <circle className="map-outer" cx="50" cy="50" r="39" /><circle className="map-mid" cx="50" cy="50" r="25" /><circle className="map-inner" cx="50" cy="50" r="11" />
    ...
    <div className="map-labels">{mapSignals.map((signal, index) => <span className={`map-label map-label-${index}`} key={signal.signal}><i className={`dot-${signal.assessment.toLowerCase()}`} />{signal.signal}</span>)}</div>
  </div>;
}
...
function Report({ lead, report }: { lead: LeadInput; report: QualificationReport }) {
  ...
  return <article className="report-shell" id="qualification-report" aria-labelledby="report-title">
    <header className="report-header">
      ...
      <div className="report-opening">
        <div className="company-block"><p className="report-eyebrow">Intelligence report</p><h2 id="report-title">{lead.company}</h2><a href={lead.website} target="_blank" rel="noreferrer">{lead.website.replace(/^https?:\/\//, "")}<ExternalLink size={14} /></a><p className="generated-date">Generated {generated}</p></div>
    ...
    <div className="report-body">
      <section className="executive-section section-rule" aria-labelledby="executive-title"><div className="section-heading"><p className="section-index">01</p><div><p className="section-kicker">Executive summary</p><h3 id="executive-title">Executive Signal</h3></div></div><div className="executive-copy"><p>The assessment finds a defined commercial and service-fit signal, with the next conversation focused on the few variables that most affect scope and timing.</p><div className="signal-blocks">{report.executiveSummary.slice(0, 3).map((finding, index) => <div key={finding.title}><span>{["FIT", "COMMERCIAL", "INTENT"][index] || "CONTEXT"}</span><h4>{finding.title}</h4><p>{finding.body}</p></div>)}</div></div></section>

      <section className="map-section section-rule" aria-labelledby="signal-map-title"><div className="section-heading"><p className="section-index">02</p><div><p className="section-kicker">Qualification signals</p><h3 id="signal-map-title">Signal Map</h3></div></div><QualificationSignalMap signals={report.signals} /><p className="map-caption">Ten defined qualification factors are shown as an analytical field. Node treatment indicates whether evidence is strong, moderate, weak or unknown.</p></section>

      <section className="factor-section section-rule" aria-labelledby="factor-title"><div className="section-heading"><p className="section-index">03</p><div><p className="section-kicker">Qualification Signals</p><h3 id="factor-title">Factor analysis</h3></div></div><div className="table-wrap"><table><thead><tr><th>Signal</th><th>Assessment</th><th>Evidence</th></tr></thead><tbody>{report.signals.map((signal, index) => <tr key={signal.signal}><td><span className="row-number">{String(index + 1).padStart(2, "0")}</span>{signal.signal}</td><td><SignalMark assessment={signal.assessment} /></td><td>{signal.evidence}</td></tr>)}</tbody></table></div></section>

      <section className="research-section section-rule"><div className="section-heading"><p className="section-index">04</p><div><p className="section-kicker">Research checklist</p><h3>What we still need to know</h3></div></div><div className="research-list">{report.missingInfo.length ? report.missingInfo.slice(0, 4).map((item, index) => <div key={item.title}><span>{String(index + 1).padStart(2, "0")}</span><h4>{item.title}</h4><p>{item.body}</p></div>) : <div><span>01</span><h4>Further validation</h4><p>The supplied brief is complete enough for the prototype assessment. Discovery remains appropriate before committing scope.</p></div>}</div></section>

      <section className="next-move-section"><div><p className="section-index">05</p><p className="section-kicker">Action plan</p><h3>Recommended next move</h3><h4>{report.recommendation.title}</h4><p>{report.recommendation.body}</p></div><ol>{report.recommendation.steps.slice(0, 3).map((step, index) => <li key={step}><span>{String(index + 1).padStart(2, "0")}</span>{step}</li>)}</ol></section>
...
export default function Home() {
  ...
  const completion = Math.round(([lead.company, lead.website, lead.serviceRequired, lead.monthlyBudget, lead.businessGoal].filter(Boolean).length / 5) * 100);

  useEffect(() => { if (!analyze.isPending) return; const timer = window.setInterval(() => setAnalysisStage((stage) => Math.min(stage + 1, analysisSteps.length - 1)), 510); return () => window.clearInterval(timer); }, [analyze.isPending]);
  const updateLead = (key: keyof LeadInput, value: string) => setLead((current) => ({ ...current, [key]: value }));

  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setFormError(""); setAnalysisStage(0); setReport(null);
  ...
  async function downloadReport() {
    ...
    try {
      ...
      while (remaining > 0) { pdf.addImage(source, "PNG", margin, margin - offset, width, height, undefined, "FAST"); remaining -= printable; offset += printable; if (remaining > 0) pdf.addPage(); }
  ...
  return <main id="top">
    <section className="hero print-hidden" aria-labelledby="hero-title">
      <div className="hero-visual" aria-hidden="true"><div className="signal-grid" /><svg viewBox="0 0 640 620" className="hero-signal-svg"><path d="M34 456C143 362 143 175 288 228s123 219 285 63" /><path d="M91 520C215 402 286 519 379 382S534 299 644 99" /><path d="M143 120c83 77 155 15 258 105s150 42 211 178" /><g className="signal-dots">{[[79,456],[142,365],[208,271],[286,228],[355,278],[392,411],[481,393],[573,295],[605,168],[150,120],[247,157],[356,226],[510,354]].map(([cx,cy], index)=><circle key={index} cx={cx} cy={cy} r={index===3 || index===8 ? 7 : 3} />)}</g></svg><div className="visual-label label-intent"><span>QUALIFIED INTENT</span><i /></div><div className="visual-label label-opportunity"><span>OPPORTUNITY</span><strong>HIGH FIT</strong></div><div className="visual-caption">ORGANIC SEARCH SIGNAL MAP <small>ABSTRACT VISUALIZATION</small></div></div>
      ...
      <div className="hero-grid container"><div className="hero-copy"><p className="hero-eyebrow">QUALIFIED ORGANIC GROWTH BEGINS WITH CLARITY</p><h1 id="hero-title">Know which SEO leads<br />are worth pursuing.</h1><p>Evaluate fit, intent, budget and business need — then turn the result into an actionable sales signal.</p><a className="hero-entry" href="#qualification-console"><span>Assess a new opportunity</span><ArrowDownRight size={17} /></a></div><div className="hero-annotation">Fit → Signal → Action</div></div>
    </section>
    ...
    <section className="tool-section print-hidden" id="qualification-console" aria-labelledby="form-title"><div className="container"><div className="console-frame"><div className="console-topline"><div><span className="console-kicker">NEW LEAD</span><span className="console-name">Qualification console</span></div><div className="completion-meter"><span>INPUT COMPLETENESS</span><i><b style={{ width: `${completion}%` }} /></i><strong>{completion}%</strong></div></div><div className="console-layout"><aside className="console-intro"><p className="section-index">01</p><h2 id="form-title">Qualify a new lead</h2><p>Give the assessment the signals it needs. The system will distinguish evidence from assumptions.</p><div className="console-note"><FileText size={16} /><span>Designed for informed outreach, not conversion prediction.</span></div></aside>
        <form className="lead-form" onSubmit={submit} noValidate><fieldset><legend><span>Lead</span><strong>Who are we evaluating?</strong></legend><div className="form-grid two-up"><label>Company<input value={lead.company} onChange={(event) => updateLead("company", event.target.value)} placeholder="Northstar Analytics" autoComplete="organization" required /></label><label>Website<div className="field-with-status"><input value={lead.website} onChange={(event) => updateLead("website", event.target.value)} placeholder="https://company.com" type="url" autoComplete="url" required />{websiteIsValid && <Check size={15} aria-label="Valid website format" />}</div></label></div></fieldset><fieldset><legend><span>Opportunity</span><strong>What are they looking for?</strong></legend><div className="form-grid"><label className="form-wide">Service required<select value={lead.serviceRequired} onChange={(event) => updateLead("serviceRequired", event.target.value)}><option>SEO strategy</option><option>Technical SEO</option><option>Content SEO</option><option>Enterprise SEO</option><option>SEO audit</option></select></label></div></fieldset><fieldset><legend><span>Commercial signal</span><strong>What is the buying context?</strong></legend><div className="form-grid two-up"><label>Monthly budget<select value={lead.monthlyBudget} onChange={(event) => updateLead("monthlyBudget", event.target.value)}><option>Under $2,000</option><option>$2,000–$4,000</option><option>$4,000–$8,000</option><option>$8,000+</option></select></label><label>Business goal<select value={lead.businessGoal} onChange={(event) => updateLead("businessGoal", event.target.value)}><option>Qualified leads</option><option>Organic revenue</option><option>Market visibility</option><option>Technical health</option></select></label></div></fieldset><button className="context-toggle" type="button" onClick={() => setExpanded((value) => !value)} aria-expanded={expanded} aria-label={expanded ? "Collapse additional context" : "Expand additional context"}>{expanded ? <Minus size={15} /> : <Plus size={15} />}<span>+ Add more context</span><ChevronDown size={16} className={expanded ? "rotated" : ""} /></button>{expanded && <fieldset className="context-fields"><legend><span>Additional context</span><strong>What will improve the assessment?</strong></legend><div className="form-grid two-up"><label>Target market<input value={lead.targetMarket} onChange={(event) => updateLead("targetMarket", event.target.value)} placeholder="United States" /></label><label>Timeline<div className="timeline-options" role="group" aria-label="Timeline">{["0–30 days", "30–90 days", "3–6 months"].map((item) => <button type="button" className={lead.timeline === item ? "active" : ""} onClick={() => updateLead("timeline", lead.timeline === item ? "" : item)} key={item}>{item}</button>)}</div></label><label className="form-wide">Current SEO challenge<textarea value={lead.seoChallenge} onChange={(event) => updateLead("seoChallenge", event.target.value)} placeholder="What has changed, what is not working, or what needs to improve?" rows={3} /></label></div></fieldset>}{formError && <p className="form-error" role="alert">{formError}</p>}<div className="form-footer"><p>Assessment output is based only on supplied information and the stated qualification framework.</p><button className="primary-button" type="submit" disabled={analyze.isPending}>{analyze.isPending ? "Reading signals" : "Qualify Lead"}<ArrowDownRight size={17} /></button></div></form></div></div></div></section>
