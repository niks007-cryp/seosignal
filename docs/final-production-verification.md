...
The production browser automation click did not initiate a network request from the hosted form; the same approved labelled submission was therefore sent through the public tRPC endpoint and was confirmed in Vercel runtime logs and Supabase. This verifies the deployed backend workflow and persistence path, but a manual browser click-through plus PDF download remains a prudent final human spot-check before external submission.
